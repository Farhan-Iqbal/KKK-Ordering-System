...
class Mapper {
    ...
    public static function generateMergeJobs($order, $details) {
        ...
        if ($packageType === '1-package') {
            $sideType = $order['side_type'] ?? 'lelaki';
            $isLelaki = ($sideType === 'lelaki');
        ...
        } else if ($packageType === '2-package') {

            // Job 1: Pakej Lelaki (ORDERID-L)
            $jobs[] = [
            ...
            // Job 2: Pakej Perempuan (ORDERID-P)
            $jobs[] = [
    ...
    private static function formatContacts($contacts) {
